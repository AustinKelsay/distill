...
Provider adapters (Codex, Claude Code, OpenCode, Droid) are specified for the rebuild but are out of scope until their dedicated tickets. Issue #22 registers those kinds for preference/detection surfaces and returns typed `unavailable` / `adapter_not_registered` without implementing parsers. Fixture remains the only concrete adapter proof for Sync Runs.
