...
- Status: in progress for desktop provider parity; CLI/native Fixture routine loop is verified, while release and human residuals remain explicit
- Rule: full product loop includes Sync Runs, Curation, and Export Artifacts.
- Current drift: the CLI and Library prove all five v1 Sources, but the desktop product path had remained Fixture-only. Issue #44 implements the bounded Tauri/React provider-neutral caller seam locally with typed Source kind/root/enabled drafts, host product-loop tests, root-removal projection survival, and mixed-source warning/redaction evidence; final-head CI/review is still pending, and packaged real-machine providers are not claimed. Durable Sync Runs, Source preferences, independent detection, lease health, warning/partial-success terminals, typed selection/lease-lost edges, transactional manual Curation, crash-recoverable Export Artifacts, cursor-paged Activity/Operations, the Library-owned parser registry and Distill-owned renormalize path (#43), the #42 CLI journey, and macOS #35/Linux #36 Fixture package smokes remain recorded in the canonical packets.
- Impacted files/modules: Library ops, curation, export, provider adapters, Tauri host/React thin callers, and packaging/cutover surfaces.
- Severity: medium — desktop provider parity is now covered at the hermetic host/bridge seam, while packaged real-provider smoke, screen-reader speech evidence, signed/notarized release packaging, Windows packaging, and Electron retirement remain explicit residuals. Rust advisory scanning is governed by the pinned CI workflow from #40; the warning inventory / non-clean boundary stays explicit rather than an unperformed gate.
- Target branch/tickets: `feature/distill-clean-rebuild`, Sync #22, Curation #24, export #25, provider #26–#29, diagnostics #30, CLI multi-Source caller #42, parser registry #43, Tauri/React multi-Source caller #44, final cutover #37, Rust advisory #40, macOS dialog focus #41.
- Acceptance criteria: async Sync Runs, transactional manual Curation, previewed crash-recoverable JSONL export, Activity/Operations diagnostics, provider adapters, thin CLI/Tauri/React callers, and macOS/Linux packaging pass the public contracts; the cutover report lists every remaining human or out-of-scope item without treating the hermetic desktop provider seam as packaged real-machine evidence.
