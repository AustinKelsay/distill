...
- Status: resolved for the hermetic desktop provider-parity seam; packaged real-provider, release, and human residuals remain explicit
- Rule: full product loop includes Sync Runs, Curation, and Export Artifacts.
- Current drift: the CLI and Library prove all five v1 Sources, and issue #44 closes the hermetic Tauri/React provider-neutral caller seam with typed Source kind/root/enabled drafts, host product-loop tests, root-removal projection survival, mixed-source warning/redaction evidence, and final-head Linux/advisory CI. Packaged real-machine providers are not claimed. Durable Sync Runs, Source preferences, independent detection, lease health, warning/partial-success terminals, typed selection/lease-lost edges, transactional manual Curation, crash-recoverable Export Artifacts, cursor-paged Activity/Operations, the Library-owned parser registry and Distill-owned renormalize path (#43), the #42 CLI journey, and macOS #35/Linux #36 Fixture package smokes remain recorded in the canonical packets.
