# Distill Electron Contract Test Matrix
...
| `raw_capture_persistence` | Validate Distill Electron-owned recoverable raw capture storage. | `test/raw-capture-contracts` |
...
- at least one Droid session JSONL with tool-use and tool-result blocks
...
| `CC-004` | `connector_contract` | Droid connector preserves transcript text while keeping thinking and tool traffic in canonical artifacts/raw records. | Parsed output includes canonical messages plus `tool_call` / `tool_result` / `raw_json` artifacts and meta raw records for non-transcript state. | Session detail can show transcript text without system-reminder noise while retaining structured provenance. | Test fails if tool traffic leaks into plain transcript text or non-transcript Droid state is dropped. | `test/connector-contract-hardening` |
...
| `RCP-002` | `raw_capture_persistence` | Virtual OpenCode capture persists recoverable raw content. | Capture row resolves to Distill Electron-owned content, not only transient process output. | Replay tooling can recover exported session JSON. | Test fails if replay depends on rerunning the source CLI. | `test/raw-capture-contracts` |
...
| `EC-002` | `export_contract` | Export includes manual curation metadata. | Export row and output include tags and labels. | Consumers can trust export metadata without re-querying Distill Electron. | Test fails if tags/labels are missing or inconsistent. | `docs/test-matrix` |
| `EC-003` | `export_contract` | Export preserves projection metadata and per-message transcript semantics. | Export payload includes session metadata plus message kind and message metadata from the current projection. | Consumers can distinguish text from meta messages, derive turn pairs from real assistant replies only, and recover session provenance without re-querying Distill Electron. | Test fails if export drops session metadata, collapses `message_kind`, pairs turns from assistant meta rows, or omits message metadata. | `impl/projection-fidelity-export` |
